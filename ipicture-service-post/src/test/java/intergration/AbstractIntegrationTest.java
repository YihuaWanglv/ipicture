...
//			requestEntity.getHeaders().set("Accept", "application/json");
//			Set<HttpMethod> set = new HashSet<HttpMethod>();
//			set.add(HttpMethod.POST);
//			requestEntity.getHeaders().setAllow(set);
			final ResponseEntity<T> entity = restTemplate.postForEntity(getBaseUrl() + requestMappingUrl, objectToPost, serviceReturnTypeClass);
			return entity.getBody();
		} catch (final Exception ex) {
			// Handle exceptions
			ex.printStackTrace();
